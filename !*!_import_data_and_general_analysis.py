...
result = find_file(file_1, file_1_path)
...
# Apply the haversine function row-wise to create a new column 'distance_miles'
df2['distance_miles'] = df2.apply(lambda row: haversine(row['oasis_template_transport → ref_latitude'], row['oasis_template_transport → ref_longitude'], (33.37), (-81.96)), axis=1)
...
def out_to_csv(df):
    ...
    final_file_path = f'/Users/Jeff/Documents/Python/{name_for_file}'
...
df2 = pd.read_csv('/Users/Jeff/Downloads/output.csv')

df2['distance_miles'] = df2.mean(numeric_only=True, axis=1)
